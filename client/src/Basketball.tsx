...
const Basketball: React.FC<BasketballProps> = ({ selectedDate, setSelectedDate }) => {
  const [scores, setScores] = useState<Score[]>([]);
  const [modalOpen, setModalOpen] = useState<boolean>(false);

  const fetchScores = async (date?: string) => {
    try {
      const res = await axios.get<Score[]>("http://localhost:3000/api/basketball", {
        params: date ? { date } : {},
      });
      setScores(res.data);
    } catch (err) {
        ...
        <DialogContent>
          <LocalizationProvider dateAdapter={AdapterDateFns}>
            <DatePicker
              ...
              minDate={new Date(2023, 9, 25)} // October 25, 2023
              maxDate={new Date(2024, 5, 18)} // June 18, 2024
              slotProps={{
                textField: {
                  fullWidth: true,
                },
